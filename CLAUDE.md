...
GitTok (repo: os-feed) is an open-source "TikTok" feed for GitHub — it aggregates the latest trends from GitHub and the AI ecosystem daily, scores them with a free LLM, and presents them as a Chinese TikTok-style feed. A GitHub Actions cron job runs at 00:00 UTC (08:00 CST) and produces bilingual (Chinese + English) digest reports, committed Markdown files, and `data/feed.json` for the React frontend.
...
- Web UI: `web/` is a React + Vite app (not the legacy `index.html`). It reads `data/feed.json` and renders a virtualized TikTok-style feed with sections (Recommended / Hot / Daily / Following + AI / Fun / Tools / Learning), like/dislike/bookmark with localStorage snapshots, weighted search, and a creator page.
- RSS Feed: `feed.xml` at the repo root. Generated by `src/generate-manifest.ts` in the same `pnpm manifest` step. Contains the latest 30 items (newest first) across all report types. Item links use hash routing: `https://chestnuts-sisyphus.github.io/os-feed/#YYYY-MM-DD/report`.
