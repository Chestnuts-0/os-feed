...
- "ai_dims": 从 [${AI_DIMS.join(", ")}] 中选 1-3 个最贴切的标签，按相关度降序排列组成数组。例如只和 AI Agent 有关：["AI Agent"]；横跨 Agent 和工具调用：["AI Agent", "工具调用"]；最多不要超过3个。判「非AI-好玩」看的是项目气质而不是类目：让人一眼觉得妙、有创意、有惊喜感、忍不住想点进去玩一玩的才算（如脑洞模拟器、奇思妙想的工具、惊艳的可视化 demo）；「带界面的正经工程」「游戏开发框架」这类不算好玩。
- "ai_score": 0 到 1 的浮点数，表示该项目与用户兴趣描述的相关度（1 = 非常相关，0 = 完全无关）。判断依据：是否 AI 相关、是否好玩有趣（按上面的好玩标准判）、是否实用能直接用、star 热度
- "summary_cn": 一句话通俗描述，用大白话说清楚这玩意是干啥的。要求：有趣、接地气、让人一眼看懂且有兴趣继续了解，必须严格 20-35 个汉字（硬性要求：少于 20 字或多于 35 个字都不合格，写完后数一遍字数确认）。像朋友给你安利一样。注意：不能和 reason_cn/detail_cn 的内容重复，必须用完全不同的角度和措辞。例如：“不用显卡也能在笔记本上跑大模型的神器”
- "reason_cn": 简要介绍，必须 110-130 个汉字（硬性要求：少于 100 字或多于 150 字都不合格，约 3 行，写完后数一遍字数确认），必须写满约 3 行（100 字以下太短不合格），多写技术细节和具体能力。一段连贯的文字。要求：比 summary_cn 更具体，可以出现专业术语和技术概念，但读者仍能在较短篇幅内看懂项目更细节的内容。不要用①②③等序号编号。不要重复 summary_cn 的内容。
